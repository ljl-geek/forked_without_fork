...
# feature: Added option to read query from file, -f option is used to do it.
# L J Laubenheimer https://github.com/ljl-geek: 2015-02-27
# feature: added -S for sqlfile, added -D for detail output
# bug: made sqfile read the whole file, line breaks and all, without eliminating newlines
# feature: added optional detail output for query with regex
# bug: made queries with regex match alert on anything over 0 if no thresholds were specified
# bug: corrected spelling of "threshold", fixed docs for query, sqlfile, detail and tablespace
# L J Laubenheimer https://github.com/ljl-geek: 2015-03-11
...
"L=s" => \$opt_L, "LD_LIBRARY_PATH=s" => \$opt_L,
"f=s" => \$opt_f, "authfile=s" => \$opt_f,
"Q=s" => \$opt_q, "query=s" => \$opt_q,
"S=s" => \$opt_sqlfile, "sqlfile=s" => \$opt_sqlfile,
...
if($opt_L) {
        $ENV{'LD_LIBRARY_PATH'}="$ENV{'LD_LIBRARY_PATH'}:$opt_L";
} 

# Output the identity of the DB (SID) if identify option is given
if($opt_l &&  $opt_I) {
        $sid="($opt_l) ";
} else {
        $sid="";
...
if($opt_l){
	...
	@out = `$cmd 2>&1`;
	if($? != 0){
		print "Failed to execute sqlplus. @out $sid";
		exit $ERRORS{'UNKOWN'};
	}
	...
	if($opt_u){
		if("@out" =~ m/Connected to/){
			print "OK - Login as user '$opt_u' successfull $sid\n";
			exit $ERRORS{'OK'};
		}
		foreach(@out){
			if( m/^ORA-/ ){
				$errmsg = trim($_);
				print "CRITICAL - Login for user '$opt_u' failed $sid - $errmsg \n";
				exit $ERRORS{'CRITICAL'};
			}
		}
		print "CRITICAL - Login for user '$opt_u' failed $sid\n";
		exit $ERRORS{'CRITICAL'};
	} else {
		if ("@out" =~ m/ORA-01017/){
...
sub check_cachehit(){
	...
	print "@ERRORS_STR[$status] - Cache hit ratio: $result% $sid\n";
	exit $status;
}
sub check_numdatafiles(){
	...
	$result = trim(@res[0]);
	$status = check_float_thresholds($result);
	print "@ERRORS_STR[$status] - Total number of data files is $result $sid\n";
	exit $status;
}
sub check_locks(){
	...
	$result = trim(@res[0]);
	$status = check_float_thresholds($result);
	print "@ERRORS_STR[$status] - Total number of locks is $result $sid\n";
	exit $status;
}
sub check_broken(){
	...
	$result = trim(@res[0]);
	$status = check_float_thresholds($result);
	print "@ERRORS_STR[$status] - Total number of broken transactions is $result $sid\n";
	exit $status;
}
sub check_fail(){
	...
	$result = trim(@res[0]);
	$status = check_float_thresholds($result);
	print "@ERRORS_STR[$status] - Total number of failed transactions is $result $sid\n";
	exit $status;
}
sub check_deferror(){
	...
	$result = trim(@res[0]);
	$status = check_float_thresholds($result);
	print "@ERRORS_STR[$status] - Total number of transactions in deferred error queue is $result $sid\n";
	exit $status;
}
sub check_invalid(){
	...
	$result = trim(@res[0]);
	$status = check_float_thresholds($result);
	print "@ERRORS_STR[$status] - Total number of invalid transations is $result $sid\n";
	exit $status;
}
sub check_query() {
	if(!($opt_q || $opt_sqlfile)) {
		print "Option 'QUERY' requires -Q <sql-query> or -S <sqlfile>\n";
		exit $ERRORS{"UNKNOWN"};
	}
	if($opt_q && $opt_sqlfile){
		print "You cannot use both command line sql query options (-Q) (or) have complex sqlquery read from a file using -S <filename>\n";
		exit $ERRORS{"UNKNOWN"};
	}
	if($opt_sqlfile){
		local(*SQLFILE, $/);   ## you have to undef your record separator $/ to get it to gulp the newlines
		open(SQLFILE, $opt_sqlfile) || my_die "Unable to open sqlfile: $opt_sqlfile\n";
		$sql_cmd = <SQLFILE>;
		#$sql_cmd =~ s/\n/ /g;  # don't whack the newlines
		close(SQLFILE);
	}

	if($opt_q){
		unless ($opt_q =~ /^([-_\@\"\\\$\*\(\)=<>,\'\% \w.]+)$/) {
			print "SQL QUERY: contains illegal characters\n (not in '-_\@\"\\\$\*\(\)=<>,\'\% \w.')\n";
			exit $ERRORS{"UNKNOWN"};
		}
                # make it decode the query if it is encoded
                if ($opt_q =~ /%[A-Fa-f0-9]{2}/) {
                        $opt_q =~ s/%([A-Fa-f0-9]{2})/chr(hex($1))/seg;
                }
	}

	$sql_cmd = $opt_q if($opt_q);
        print "\t ## Raw SQL Query from commandline option(-Q) is $sql_cmd\n" if($opt_q && $DEBUG);
	$sql_cmd =~ s/;?$/;/;

	print "\t ## SQL Query from Sqlfile($sqlfile) is $sql_cmd\n" if($opt_f && $DEBUG);
	print "\t ## SQL Query from commandline option(-Q) is $sql_cmd\n" if($opt_q && $DEBUG);

	if($opt_s) {
		unless ($opt_s =~ /^([-_\@\"\%:=\(\)\' \w.]+)$/) {
			print "Output string you are asking to match contains illegal chars\n (not in '-_\@\"\%:=\(\)\' \w.')\n";
			exit $ERRORS{"CRITICAL"};
		}
		$outStr = $opt_s;
	} else {
	...
	# Executing Sqlplus to run sql query #
	...
	if($opt_r){
		...
		if (!($opt_w || $opt_c)) {
			# if there is no threshold, set it to zero so any match alerts 
			$opt_c = 0;   
		}
		$numhits = 0;
		if(@res2 = grep(/$opt_r/,@res)){
			print join("",@res2) if $DEBUG;
			chomp(@res2);
			$bad = ": ".join("; ",@res2) if $opt_D;
			$numhits = scalar @res2;
			$status = check_float_thresholds($numhits);
		} else {
			$status = 0;
		}
		print "@ERRORS_STR[$status] - $outStr $numhits $bad $sid\n";
		exit $status;
	}
	if($opt_w || $opt_c) {
		$status = check_float_thresholds($resStr);
		if (looks_like_number($resStr)) {
			print "@ERRORS_STR[$status] - $outStr $resStr $sid|result=$resStr;$opt_w;$opt_c\n";
		} else {
			print "@ERRORS_STR[$status] - $outStr $resStr $bad $sid\n";
		}

		exit $status;
	}
	print "QUERY option requires a search-string and/or numeric thresholds $sid\n";
	exit {"UNKNOWN"};
}
...
sub check_archiving(){
	...
	if($logmode eq "ARCHIVELOG") {
		print "OK - Log mode is 'ARCHIVELOG' $sid\n";
		exit $ERRORS{"OK"};
	} else {
		print "CRITICAL - Log mode is '$logmode' $sid";
		exit $ERRORS{"CRITICAL"};
	}
}
...
sub check_datafile(){
	if($opt_a){
		...
		@filearr = split("/", $file);
		$status = check_float_thresholds($result);
		print "@ERRORS_STR[$status] - Latest datafile (@filearr[$filearr-1]) for tablespace '$opt_a' is $result% full  $sid\n";
		exit $status;
	}
	print "Option DATAFILE requires a tablespace argument $sid\n";
	exit $ERRORS{"UNKNOWN"};
}
...
sub check_extents(){
	if($opt_a){
		...
		$result = trim(@res[0]);
		$status = check_float_thresholds($result);
		print "@ERRORS_STR[$status] - Tablespace '$opt_a' has $result extents left $sid\n";
		exit $status;
	} else {
		...
		if($opt_c){
			...
			print "Option 'EXTENTS' requires a warning _or_ a critical threshold $sid\n";
			exit $ERRORS{"UNKNOWN"};
		}
		$threshold_min = (split(":", $threshold))[0];
		$threshold_max = (split(":", $threshold))[1];
		if($threshold_min eq "" or $threshold_min eq "~")
		{
			undef $threshold_min;
		}
		if($threshold_max eq ""){undef $threshold_max;}
		my $sql_threshold = "";
		if(defined($threshold_min))
		{
			$sql_threshold .= "TOTAL_EXTENTS <= $threshold_min";
			if(defined($threshold_max)){$sql_threshold .= " and ";}
		}
		if(defined($threshold_max))
		{
			$sql_threshold .= "TOTAL_EXTENTS >= $threshold_max";
		}
		$sql_cmd = "select TABLESPACE_NAME from dba_free_space_coalesced where ".$sql_threshold.";";
		@res = exec_sql_cmd($sql_cmd);
		$num=@res;
		if(@res[0] eq ''){
			$num=0;
		...
		if($num == 0){
			print "OK - All tablespaces have ".range_compare_helper_inside($threshold, "more than")." $threshold extents left $sid\n";
			exit $ERRORS{"OK"}
		} elsif ($opt_c) {
			print "CRITICAL - $num tablespaces have ".range_compare_helper_outside($threshold, "less than")." $threshold extents left $sid. ($result_str)\n";
			exit $ERRORS{"CRITICAL"};
		} elsif ($opt_w) {
			print "WARNING - $num tablespaces have ".range_compare_helper_outside($threshold, "less than")." $threshold extents left $sid. ($result_str)\n";
			exit $ERRORS{"WARNING"};
		}

	}
	print "Unknown error $sid\n";
	exit $ERRORS{"UNKNOWN"};
}
...
sub check_tablespace(){
	if($opt_a){
		...
		$result = trim(@res[0]);
		$status = check_float_thresholds($result);
		print "@ERRORS_STR[$status] - Tablespace '$opt_a' is $result% full $sid\n";
		exit $status;
	} elsif($opt_e){
		...
		if($opt_c){
			...
			print "Option 'TABLESPACE' requires a warning _or_ a critical threshold $sid\n";
			exit $ERRORS{"UNKNOWN"};
		}
		$opt_e =~ s/,/','/g;
		$threshold_min = (split(":", $threshold))[0];
		...
		if($threshold_min eq "" or $threshold_min eq "~"){undef $threshold_min;}
		if($threshold_max eq ""){undef $threshold_max;}
		my $sql_threshold = "";
		if(defined($threshold_min))
		{

			$sql_threshold .= "((a.BYTES-b.BYTES)/a.BYTES*100) <= $threshold_min";
			if(defined($threshold_max)){$sql_threshold .= " or ";}
		}
		if(defined($threshold_max))
		{
		...
		$result_str = format_multi(@res);
		if($num == 0){
			print "OK - All tablespaces use ".range_compare_helper_inside($threshold, "less than")." $threshold% $sid\n";
			exit $ERRORS{"OK"}
		} elsif ($opt_c) {
			print "CRITICAL - $num tablespaces use ".range_compare_helper_outside($threshold, "more than")." $threshold% $sid ($result_str)\n";
			exit $ERRORS{"CRITICAL"};
		} elsif ($opt_w) {
			print "WARNING - $num tablespaces use ".range_compare_helper_outside($threshold, "more than")." $threshold% $sid ($result_str)\n";
			exit $ERRORS{"WARNING"};
		}
	} else {
		# Check all tablespaces
...
sub check_backupmode(){
	if($opt_a){
		...
		if(trim(@res[0]) eq $opt_a){
			print "CRITICAL - Tablespace '$opt_a' is in backupmode $sid\n";
			exit $ERRORS{"CRITICAL"};
		} else {
			print "OK - Tablespace '$opt_a' is not in backupmode $sid\n";
			exit $ERRORS{"OK"};
		}
	} else {
		# Check all tablespaces
		...
		$num = @res;
		if(trim(@res[0]) eq ""){
			print "OK - No tablespaces in backupmode $sid\n";
			exit $ERRORS{"OK"};
		} else {
			$resStr = format_multi(@res);
			print "CRITICAL - $num tablespaces are in backupmode $sid ($resStr)\n";
			exit $ERRORS{"CRITICAL"};
		}
	}
}
...
sub check_datafile_bytes() {
	if($opt_a){
		...
		@filearr = split("/", $file);
		$status = check_float_thresholds($result);
		print "@ERRORS_STR[$status] - Latest datafile (@filearr[$filearr-1]) for tablespace '$opt_a' has $result MB left $sid\n";
		exit $status;
	}
	print "Option DATAFILE requires a tablespace argument $sid\n";
	exit $ERRORS{"UNKNOWN"};
...
sub check_extents_table() {
	if($opt_a){
		...
		$result = trim(@res[0]);
		$status = check_float_thresholds($result);
		print "@ERRORS_STR[$status] - Table '$opt_a' has $result extents left $sid\n";
		exit $status;
	} else {
		# Check all tablespaces
		if($opt_c){
			...
			print "Option 'EXTENTS_TABLE' requires a warning _or_ a critical threshold $sid\n";
			exit $ERRORS{"UNKNOWN"};
		}
		$threshold_min = (split(":", $threshold))[0];
		$threshold_max = (split(":", $threshold))[1];
		...
		if(defined($threshold_min))
		{
			...
			$sql_threshold .= "(max_extents - extents) <= $threshold_min";
			if(defined($threshold_max)){$sql_threshold .= " or ";}
		}
		if(defined($threshold_max))
		{
			$sql_threshold .= "(max_extents - extents) >= $threshold_max";
		}
		$sql_cmd = "select SEGMENT_NAME from dba_segments where SEGMENT_TYPE like 'TABLE' AND $sql_threshold;";
		@res = exec_sql_cmd($sql_cmd);
		$num=@res;
		if(@res[0] eq ''){
			$num=0;
		...
		$result_str = format_multi(@res);
		if($num == 0){
			print "OK - All tables have ".range_compare_helper_inside($threshold, "more than")." $threshold extents left $sid\n";
			exit $ERRORS{"OK"}
		} elsif ($opt_c) {
			print "CRITICAL - $num tables have ".range_compare_helper_outside($threshold, "less than")." $threshold extents left $sid. ($result_str)\n";
			exit $ERRORS{"CRITICAL"};
		} elsif ($opt_w) {
			print "WARNING - $num tables have ".range_compare_helper_outside($threshold, "less than")." $threshold extents left $sid. ($result_str)\n";
			exit $ERRORS{"WARNING"};
		}
	}
	print "Unknown error\n";
...
sub print_usage () {
	...
	print "    a list of all that exceed warning or critical threshold are presented.\n";
	...
	print " $PROGNAME -o QUERY -l <SID> -u <user> -p <password> \n";
	print "		  -Q <query> (or) -S <filename which contains sql query> \n";
	print "           [-s <output-string>] [-r <search-string>] [-w <warn>] [-c <crit>] [-D]\n";
	...
	print "    -D when used with -r gives the detail of the matches\n"; 
	...
	print "  -I can be addedd to make the output include the SID (-l) in the plugin output.  \n";
	print "  This can be useful if there are multiple SIDs running on a given host\n";
	print "\n";
	print " Examples:\n";
...
# Compare a float to supplied warning/critical thresholds
sub check_float_thresholds {
	my $res = shift;
	$res = trim($res);
	...
	$res =~ s/,/./;
	if($DEBUG){
		print "Comparing value '$res' to thresholds\n";
	}
